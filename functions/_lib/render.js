...
const MARKED_SCRIPT = "https://registry.npmmirror.com/marked/17.0.1/files/lib/marked.umd.js";
const PURIFY_SCRIPT = "https://registry.npmmirror.com/dompurify/3.3.1/files/dist/purify.min.js";
...
// 便签预览页：正文以内嵌 JSON 提供，由 /assets/js/note.js 在浏览器端渲染。
export function notePage(note) {
    return new Response(noteHtml(note), {
        headers: {
...
export function markdownFile(slug, markdown, headOnly) {
...
function noteHtml(note) {
    ...
    const createdAtHtml = note.createdAt
...
<body>
    ...
    <main class="note-shell page-width">
        <article class="markdown" id="noteContent" aria-live="polite">
    ...
    <script type="application/json" id="noteData">${jsonForScript({ markdown: note.markdown })}</script>
</body>
</html>`;
}
